  ...
  <div class="product-list">
    ...
    <div id="each-store" v-for="store in stores" :key="store.store_id" >
     ...
     <h1 id="store-title" class="list-items-title">
        <img :src="`./stores/static/${store.store_id}/logo.svg`" class="company-svg"/>
        {{store.store_name}}
...
export default {
  ...
  computed: {
    ...
    tagchoice() {
      return this.$store.state.tagchoice[0];
    },

    products() {
      
      var products = this.$store.state.stores.map((stores, index) => {
        var result = stores.products.filter((prod) => { 
          
          
          return this.$store.state.sale ? prod.price < this.$store.state.highprice && prod.sale : prod.price < this.$store.state.highprice;});
        ...
        return result;
            ...
            //  console.log('result.length: ', checker);
...
.company-svg {
  width: 2.8rem;
  min-width: 1.8rem;
  filter: grayscale(0.6) brightness(1.8) saturate(2) contrast(0.8);
