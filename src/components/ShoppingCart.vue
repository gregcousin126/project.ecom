...
<!-- </div> -->
<!-- <button class="btn-splatter" v-on:click="makeSplatter(event)" text="PRESS ME"> 
  {{text}}
  <div v-for="splatter, i in splatters" v-bind:class="splatter.type" v-bind:style="splatter.style" v-bind:ref="createRef(i)" ></div>
</button> -->
<!-- export default { -->
<div class="button-explode"><button id="exploding-button" class="btn-splatter" v-on:click="makeSplatter($event)" text="PRESS ME">{{text}}<div v-for="splatter, i in splatters" v-bind:class="splatter.type" v-bind:style="splatter.style" v-bind:ref="createRef(i)" ></div></button></div>
<!-- <ButtonExplode text="PRESS ME" @click="ButtonExplode.makeSplatter(event)"/> -->
...
makeSplatter: function(e) {
  // let el = document.getElementById('exploding-button').target
...
/* #app {font-family: 'Open Sans', sans-serif;font-size: 24px;background-image: linear-gradient(to bottom right,#ff4e4e 0 15%, #f8ff64 40% 60%, #00ce1a 85% 100%);width: 100%;height: 100%;display: flex;align-items: center;justify-content: center;} */
.btn-splatter {color: #ffffff;font-family: inherit;font-weight: bold;background-color: #4487ed;width: 12em;height: 4em;border: 0;outline: 0;padding: 0;margin: 0;position: relative;border-radius: 0.9em;box-shadow: 0 0.6em 0.9em rgba(0, 0, 0, 0.6);user-select: none; /* Thanks to @SplittyDev */transform: translateY(-0.5em);transition:background-color 300ms cubic-bezier(0.18, 0.89, 0.32, 1.28),box-shadow 300ms cubic-bezier(0.18, 0.89, 0.32, 1.28),transform 300ms cubic-bezier(0.18, 0.89, 0.32, 1.28);}
.btn-splatter:focus {outline: 0;}
.btn-splatter:active {background-color: #2770df;box-shadow: 0 0 0 rgba(0, 0, 0, 0.5);transform: translateY(0em) scale(0.9);transition:background-color 150ms ease-out,box-shadow 150ms ease-out,transform 150ms ease-out;}
.splatter {position: absolute;top: 0;left: 0;}
.splatter-round, .splatter-ring, .splatter-star {position: absolute;}
.splatter-round {width: 1em;height: 1em;background-color: #1da5ff;border-radius: 50%;}
.splatter-ring {width: 0.8em;height: 0.8em;border: solid 0.35em #0099e6;border-radius: 50%;}
...
.coin-svg {
-webkit-animation: float-coin 3s ease-in-out infinite;
    ...
    margin: 9px 0px 18px 4px;
    /* min-width: 0.8rem; */
    /* font-size: 17px; */
    display: inline-grid;
    position: relative;
    -webkit-filter: drop-shadow(1px 2px 3px gold) contrast(.95) grayscale(0) saturate(.85);
    filter: drop-shadow(0px 1px 3px gold) contrast(.95) grayscale(0) saturate(.85);
}
.coin-container {
width: 100%;
    grid-gap: 5px;
    ...
    /* font-size: 17px; */
    display: inline-grid;
    /* filter: grayscale(1) brightness(1.52) contrast(2); */
